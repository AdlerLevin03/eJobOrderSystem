...
$personnel = [];
try {
    $query = "SELECT * FROM personnels WHERE id = ?";
    ...
    echo "Error fetching personnel: " . $e->getMessage();
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle photo upload (optional) - if no new photo uploaded, keep existing
    ...
    if (empty($fname) || empty($lname) || empty($gmail) || empty($cpno) || empty($department_id) || empty($position_id)) {
        ...
        try {
            $check_query = "SELECT id FROM personnels WHERE gmail = ? AND id != ?";
            $check_stmt = $pdo->prepare($check_query);
            $check_stmt->execute([$gmail, $personnel_id]);
            ...
            if ($check_stmt->rowCount() > 0) {
                ...
                // Update personnel
                try {
                    $update_query = "UPDATE personnels SET fname = :fname, mname = :mname, lname = :lname, suffix = :suffix, prefix = :prefix, gmail = :gmail, cpno = :cpno, department_id = :department_id, position_id = :position_id, photo = :photo, status = :status WHERE id = :id";
                    $update_stmt = $pdo->prepare($update_query);
                    $update_stmt->bindValue(':fname', $fname);
                    $update_stmt->bindValue(':mname', $mname);
                    $update_stmt->bindValue(':lname', $lname);
                    $update_stmt->bindValue(':suffix', $suffix);
                    $update_stmt->bindValue(':prefix', $prefix);
                    $update_stmt->bindValue(':gmail', $gmail);
                    $update_stmt->bindValue(':cpno', $cpno);
                    $update_stmt->bindValue(':department_id', $department_id);
                    $update_stmt->bindValue(':position_id', $position_id);
                    if ($photo !== null) {
                        $update_stmt->bindValue(':photo', $photo, PDO::PARAM_LOB);
                    } else {
                        $update_stmt->bindValue(':photo', null, PDO::PARAM_NULL);
                    }
                    $update_stmt->bindValue(':status', $status);
                    $update_stmt->bindValue(':id', $personnel_id, PDO::PARAM_INT);
                    $update_stmt->execute();
                    ...
                    $error_message = 'Error updating personnel: ' . $e->getMessage();
                }
            }
        } catch (PDOException $e) {
...
<head>
    ...
    <title>Edit Personnel - Free WiFi Job Order System</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <main>
        <div class="container">
            ...
            <div class="page-header">
                ...
                <p>Update personnel member information</p>
            </div>
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                        ...
                        <div class="form-group full-width">
                            ...
                            <?php if (!empty($photo)): ?>
                                ...
                                <div style="margin-bottom:10px;">
                                    <img src="data:<?php echo htmlspecialchars($mime); ?>;base64,<?php echo $base; ?>" alt="photo" style="height:80px;border-radius:8px;border:1px solid var(--border-color);">
                                </div>
                            <?php endif; ?>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group full-width">
                            ...
                            <input 
                                ...
                                required
                            >
                            <div class="char-counter" id="charCounter"><?php echo strlen($cpno); ?>/11 digits</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-submit">💾 Update Personnel</button>
                </div>
            </form>
        </div>
    </main>
    ...
    <script>
        ...
        const editPhotoInput = document.getElementById('photo');
        if (editPhotoInput) {
